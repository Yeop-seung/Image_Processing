...
namespace RtspClientExample
{
    class Channel
    {
        ...
        [DllImport("d3dxrender.dll", CallingConvention = CallingConvention.Cdecl)] static extern void D3DXRenderDraw([MarshalAs(UnmanagedType.SysUInt)] IntPtr pHandle, IntPtr hWnd, IntPtr pData, int nSize, int nVideoWidth, int nVideoHeight, bool bIsLock);
        ...
        //프레임 큐 버퍼 할당
        CQueueBuffer frameBuffer;

        //RTSP 초기화
        RtspReceiver rtsp;

        //D3DX 핸들
        public IntPtr pD3DXHandle;

        //Windows 핸들
        public IntPtr pWinHandle;
        ...
        public sealed unsafe class VideoDecoder
        {
            ...
            public int Decode(ref byte[] videoFrame, IntPtr pD3DXHandle, IntPtr pWinHandle)
            {
                AVPacket packet;
                int gotPicture = 0;
                ...
                unsafe
                {
                    ...
                    fixed (byte* pVideo = videoFrame)
                    {
                        ...
                        if (gotPicture == 1)
                        {
                            ...
                            IntPtr pointer = pinnedArray.AddrOfPinnedObject();                           // Do your stuff...

                            D3DXRenderDraw(pD3DXHandle, pWinHandle, pointer, 1920 * 1080 * 3 / 2, 1920, 1080, false);
                            ...
                            if (m_DebugFileDump==false )
                            {
                                ...
                                //D3DXRenderDraw(pD3DXHandle, pWinHandle, fra1920 * 1080 * 3 / 2, 1920, 1080, false);
                                //D3DXRenderDraw(pD3DXHandle, pWinHandle, 720 * 480 * 3 / 2, 720, 480, false);
                                ...
                                if ((AVPixelFormat)m_frame->format == AVPixelFormat.AV_PIX_FMT_YUVJ420P)
                                {
                                    //Save YUV
                                    {
                                        ...
                                        //FileStream outFs = new FileStream("Dump.yuv", FileMode.CreateNew | FileMode.Truncate);
        ...
        static void DecodingThread(object arg)
        {
            ...
            ref CQueueBuffer frameBuffer = ref param.frameBuffer;
            IntPtr pD3DXHandle = param.pD3DXHandle;
            IntPtr pWinHandle = param.pWinHandle;
            ...
            while (g_Run)
            {
                ...
                if (videoFrame != null)
                {
                    decoder.Decode(ref videoFrame, pD3DXHandle, pWinHandle);
                }
                else
                {
        ...
        {
            public CQueueBuffer frameBuffer;
            public IntPtr pD3DXHandle;
            public IntPtr pWinHandle;

            public Params_T(ref CQueueBuffer fb, IntPtr ptr, IntPtr pWin)
            {
                frameBuffer = fb;
                pD3DXHandle = ptr;
                pWinHandle = pWin;
        ...
        public void ServiceStart(IntPtr winHandle)
        {
            String url = "rtsp://admin:olzetek1234@173.208.195.45:554/ISAPI/streaming/channels/01";

            // 윈도우 핸들
            pWinHandle = winHandle;

            // Direct3D 핸들 
            pD3DXHandle  = (IntPtr)D3DXRenderCreate(pWinHandle, 1920, 1080, false);

            //프레임 큐 버퍼 할당
            frameBuffer = new CQueueBuffer();

            Params_T param = new Params_T(ref frameBuffer, pD3DXHandle, pWinHandle);
            ...
            //RTSP 초기화
            rtsp = new RtspReceiver(ref frameBuffer);

            //스트리밍 시작
            rtsp.Start(url);
        }
        public void ServiceStop(IntPtr winHandle)
        {
            rtsp.Stop();
